    ...
    <Card rarity={card.stats.rarity} isFavorite={card.isFavorite} className="absolute inset-0 flex h-full w-full flex-col overflow-hidden rounded-xl [backface-visibility:hidden] [-webkit-backface-visibility:hidden] [transform:rotateY(180deg)]">
      ...
      <CardContent className="p-3 sm:p-4 flex-1 flex flex-col justify-between">
        <div className="space-y-2 sm:space-y-4">
          <p className="text-[10px] sm:text-xs italic text-muted-foreground line-clamp-2 sm:line-clamp-3">{card.description}</p>
        ...
        <Button variant="destructive" size="sm" className="w-full mt-2 sm:mt-4 h-7 sm:h-9 text-[10px] sm:text-xs" onClick={onDelete}>
